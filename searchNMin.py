...
for i in range(len(winners)):
    for j in range(len(candidates)):
        ...
        Nmin[i,j] = findMinNvoters(candidates[j], Nwinner = winners[i], Nsubset = 5, Ngrades = 5, q = 1000, alpha = 2, Ntests=500)
        print("Nmin: %i\n" % Nmin[i,j])

np.savetxt('nmin-winners-1-5-10-candidates-10-to-100.txt', Nmin)
